...
####################################################################################
####################################################################################
#data.MSM=simData[it==1&status==1];LMp=LMps;base.covs=NULL;idOrig="nummer"
prepare.LMData<-function(data.MSM,LMp=LMps,base.covs=NULL, idOrig="nummer"){
  ...
  data.MSM[,c("entry","exit","event"):=list(min(Tstart),max(Tstop),max(as.numeric(to))),by=c(idOrig,"it","scenario")]
  ############################################################################
  LMall<-NULL
  ...
  LMlong <- msprep(time =c(NA,rep("Tstop",3)), status =c(NA,paste0("stat",1:3)), data = LMall, 
                   keep = c(idOrig,"entry","LM","scenario","it",covs), trans = tmatCompete)
...
pt.pred<-function(exposure,pts,LM,pstats=2:4){
